...
const PLAN = fs.readFileSync(
    path.join(__dirname, "fixtures/plan.md"),
    "utf8",
);
...
test.describe("core logic", () => {
            ...
            for (const rng of ranges) {
                if (rng.kind === "thread") {
                    if (!lines[rng.start - 1].startsWith("<!-- marginalia:thread"))
                        ok = false;
                    if (lines[rng.end - 1] !== "<!-- /marginalia:thread -->")
                        ok = false;
                }
            }
            return { ok, totalLines: lines.length, lastEnd: ranges[ranges.length - 1].end };
        }, COMPLICATED);
        expect(r.ok).toBeTruthy();
        expect(r.lastEnd).toBe(r.totalLines);
    });

    test("agent prompt: saved-in-place mode references the file, no full content", async ({ page }) => {
        const r = await page.evaluate((src) => {
            const m = window.__marginalia;
    ...
    test("agent prompt: not-saved mode includes the full document", async ({ page }) => {
        const r = await page.evaluate((src) => {
            const m = window.__marginalia;
            m.state.doc = m.parseMarkdown(src);
    ...
    test("agent prompt lists only unresolved threads and notes resolved ones", async ({ page }) => {
        const r = await page.evaluate((src) => {
            const m = window.__marginalia;
            m.state.doc = m.parseMarkdown(src);
            m.state.planPath = "plan.md";
...
test.describe("UI", () => {
    ...
    test("add a line comment via the floating +, then reply and resolve", async ({ page }) => {
    ...
    test("download contains the marginalia markup and round-trips", async ({ page }) => {
        // add a comment first
        await page.locator(".block").first().hover();
        await page.waitForTimeout(150);
        await page.locator("#floatingAdd").click({ force: true });
        ...
        const r = await page.evaluate((src) => {
            ...
            const doc = m.parseMarkdown(src);
            return {
                stable: m.serializeDoc(doc) === m.serializeDoc(m.parseMarkdown(m.serializeDoc(doc))),
    ...
    test("copy agent prompt puts a structured prompt on the clipboard", async ({ page }) => {
        await page.locator(".block").first().hover();
        await page.waitForTimeout(150);
        await page.locator("#floatingAdd").click({ force: true });
    ...
    test("reset clears browser state and returns to the empty state", async ({ page }) => {
